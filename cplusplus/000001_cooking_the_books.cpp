 ...
 * will fail to notice two digits being swapped in any given number, but any discrepancy more egregious will certainly be
 * caught. It's also painfully obvious when a number has fewer digits than it ought to, so a bad accountant will never
 * swap the first digit of a number with a 0.
 *
 ...
 *
 * Output:
 * For the ith number, print a line containing "Case #i: " followed by the smallest and largest numbers that can be made from
 * the original number N, using at most a single swap and following the rules above.
 *
 * Example 1:
 * Input:
 * 5
